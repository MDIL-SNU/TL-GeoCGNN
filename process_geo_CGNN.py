...
def read_orthogonal_array_row_i(num_row):
    ...
    keys = array[0].split(',')
    values = list(map(float, array[num_row].split(',')))

    lr_multiplier = 1 if "lr" not in keys else values[keys.index("lr")]
    
    frozen_layer_dict = {}
    layer_count_dict = {}
    for k, v in zip(keys, values):
        if "lr" not in k:
            if k not in frozen_layer_dict.keys():
                frozen_layer_dict[k] = []
                layer_count_dict[k] = 0

            if v == 1:
                frozen_layer_dict[k].append(layer_count_dict[k])

            layer_count_dict[k] += 1

    return frozen_layer_dict, lr_multiplier


def create_model(device, model_param, optimizer_param, scheduler_param, load_model, orthogonal_row):
    model=geo_CGNN(**model_param)
    
    if load_model:
        if orthogonal_row > 0:
            frozen_layer_dict, lr_multiplier = read_orthogonal_array_row_i(orthogonal_row)
        
            for layer_name, frozen_layer_idx in frozen_layer_dict.items():
                for idx, child in enumerate(model.__dict__["_modules"][layer_name].children()):
                    if idx in frozen_layer_idx:
    ...
    if optim_name == "sgd":
        ...
        if load_model and orthogonal_row > 0:
            optimizer = optim.Adam([{'params' : filter(lambda p: p.requires_grad, model.embedding.parameters()), 'lr': optimizer_param["lr"]*lr_multiplier},
                                    {'params' : filter(lambda p: p.requires_grad, model.conv.parameters()), 'lr': optimizer_param["lr"]*lr_multiplier},
                                    {'params' : filter(lambda p: p.requires_grad, model.MLP_psi2n.parameters()), 'lr': optimizer_param["lr"]*lr_multiplier},
                                    {'params' : filter(lambda p: p.requires_grad, model.gated_pooling.parameters()), 'lr': optimizer_param["lr"]},
                                    {'params' : filter(lambda p: p.requires_grad, model.linear_regression.parameters()), 'lr': optimizer_param["lr"]}])
        else:
...
def main(device, model_param, optimizer_param, scheduler_param, dataset_param, dataloader_param,
         num_epochs, seed, load_model,pred,pre_trained_model_path, orthogonal_row):
    ...
    if test_ratio==-1:
        ...
        indices = list(range(n_graph))
        split = {"train": indices[0:n_graph-39], "val": indices[0:n_graph-39], "test": indices[n_graph-39:n_graph]}
    else:
    # normal case
    ...
    model = create_model(device, model_param, optimizer_param, scheduler_param, load_model, orthogonal_row)
    if load_model:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--orthogonal_row", type=int, default=-1)
    ...
    main(device, model_param, optimizer_param, scheduler_param, dataset_param, dataloader_param,
         options["num_epochs"], options["seed"], options["load_model"],options["pred"],options["pre_trained_model_path"], options["orthogonal_row"])
